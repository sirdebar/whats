...
def request_sms(call):
    ...
    if worker_id:
        ...
        request_msg = bot.send_message(
            worker_id,
            f"🔔 <b>Напишите смс к номеру {service_emoji} {service.capitalize()} {number}</b> \n \n"
...
def decrement_counter_handler(call):
    ...
    if worker_id:
        service = db.get_service_by_number(number)
        bot.send_message(worker_id, f"⚠️ <b>Номер {service.capitalize()} {number} слетел! (-1)</b>")
...
def finalize_number_status(number, chat_id, message_id):
    ...
    if number_info:
        ...
        if success == 0 and failed == 0:
            ...
            if worker_id:
                service = db.get_service_by_number(number)
                bot.send_message(worker_id, f"✅ <b>Ваш номер {service.capitalize()} {number} был успешно засчитан! (+1)</b>")
        else:
            db.execute_query("DELETE FROM numbers WHERE number_id = ?", (number_id,))
            bot.edit_message_reply_markup(chat_id, message_id, reply_markup=None)
            update_message_with_numbers(chat_id, issued_to)
